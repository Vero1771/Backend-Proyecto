...
router.post('/ingresar', checkLoginAdmin, function (req, res, next) {
  const { categorias = [], directores = [], ...datosPelicula } = req.body;

  Peliculas_Controller.ingresar_pelicula(datosPelicula, categorias, directores)
    .then(r => res.status(r.code).json(r))
...
router.put('/editar/:id', checkLoginAdmin, function (req, res, next) {
  ...
  Peliculas_Controller.editar_pelicula(id, datosPelicula, categorias, directores)
    .then(r => res.status(r.code).json(r))
    .catch(err => res.status(err.code || 500).json(err));
            ...
            .then((directores) => {
              res.render(
                './peliculas_views/ingresar_peliculas',
                {
                  ...
                  directores_list: directores.result
                }
              );
            })
            .catch(err => {
